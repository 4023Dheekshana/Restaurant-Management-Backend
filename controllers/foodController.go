...
func GetFoods() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		projectStage := bson.D{
			{
				Key: "$project", Value: bson.D{
					...
					{Key: "food_items", Value: bson.D{{Key: "$slice", Value: []interface{}{"$data", startIndex, recordPage}}}},
				},
			},
		}
		...
		defer cancel()
		if err != nil {
			c.JSON(http.StatusInternalServerError, gin.H{"error": "error occured while feetching food items"})
			return
		}
		var allFoods []bson.M
...
func GetFood() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.JSON(http.StatusBadRequest, gin.H{"error": "error occured while fetching the food item"})
			return
		}
